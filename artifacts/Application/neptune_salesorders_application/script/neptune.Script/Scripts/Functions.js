...
function showApproved() {
    var binding = TableSalesOrders.getBinding("items");
...
function submit() {
    ...
    ButtonViewSalesOrderPDF.setVisible(true);
    ButtonViewSalesOrderPDF.setText("View Sales Order: " + final_data.ORDERNUMBER);
    ButtonViewSalesOrderPDF.setTooltip(final_data.ORDERNUMBER);

    var options = {
        data: final_data,
    };

    apiRestAPIPOST(options);
    ...
    Push.create("New Sales Order", {
        ...
        icon: "/media/root/neptune-salesorders-media/neptune-logo.png",
        timeout: 6000,
        onClick: function () {
            window.focus();
            this.close();
